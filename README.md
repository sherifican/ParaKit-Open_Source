...
> **Version in this release:** `4.11.1`  •  **Runtime:** Python **3.12** (required)
...
| Version | Summary |
|---|---|
| **v4.11.1**<br>2026-08-22 | <ul><li>**Updates no longer look frozen while they download.** The update window listed files that were already current but said nothing at all while it downloaded new ones, so a release carrying many new files - 4.11.0's icon set is about 1,100 of them - left the counter parked on the last "up to date" line for several minutes with the Close button disabled. Each file now appears as it downloads, the counter moves with it, and installing is listed separately.</li><li>**Warnings in the update notice stand out.** When a release needs to say something before you press Download, that paragraph is now shown in bright yellow at the top of the What's new preview instead of the same grey as everything else.</li><li>**The Waveform view has its own render settings.** The Spectral Comparison tab's Render button only ever applied to the Spectrogram, so it did nothing on the Waveform view. That view now has its own panel behind the same button: Style (Filled, Hollow or Bars), two-tone stereo colors, an amplitude scale (boosted, linear or decibel) and a gain slider.</li><li>**Small fix:** the Send to Spectral buttons on the Audio to MIDI, MIDI Editor and Song Tester tabs still showed the old chart icon after the tab moved to the spectrum-bars icon.</li></ul> |
| **v4.11.0**<br>2026-08-22 | <ul><li>**Heads-up before you update: this one takes a few minutes and looks stuck.** It adds about 1,100 small icon files, and update windows before 4.11.1 only move their counter for files that are already up to date - so the window sits at about <code>23/1200</code> for roughly five minutes while it downloads the rest. It is working; leave it running. Nothing on the install changes until every file has downloaded and been verified, and the Close button returns when it finishes.</li><li>**Crisp icons across the whole app.** Every button, checkbox, section heading, right-click menu item, and tab now carries a crisp Fluent System Icon instead of a blurry color-emoji glyph. Icons are state-aware - Play/Pause buttons, the Dark/Light theme toggle, and the song-library Play chips swap their icons live. The set is Microsoft's MIT-licensed Fluent icons, pre-rendered at exact pixel sizes for the dark theme, with a plain-text fallback if the icons folder is missing.</li><li>**Song Tester now tests a finished .rlrr alone.** The Sync Test previously refused to run without a MIDI file even when a .rlrr was loaded. It now runs from a .rlrr plus audio: BPM comes from the chart itself, the report is labeled by the actual source, and Auto Fetch Audio reads the chart's own audio list first, filling the audio fields from the files beside it.</li><li>**Song Tester .rlrr analysis fixed.** Testing with a .rlrr reported "Total notes (.rlrr): 0" and an all-zero instrument balance, because chart event times are stored as text and instrument names use internal class names. Both are read correctly now.</li><li>**Small fix:** the small screen icon on the Preview buttons rendered with its top edge cut off; glyphs are now positioned by their actual drawn shape.</li></ul> |
