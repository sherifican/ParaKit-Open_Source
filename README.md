...
| Version | Summary |
|---|---|
| **v4.9.13**<br>2026-08-10 | <ul><li>**Note volume is now a choice, and it starts switched off.** The previous release began writing a volume for every note, taken from how hard the drum was struck, and gave nobody the option. Audio to MIDI now carries a **Vary note volume to match the playing** checkbox. Off is the default because the two directions are not equally easy to undo: adding variation to a flat chart is one setting away, while removing it from a chart you did not want varied means editing every note by hand. Unticked, conversions come out exactly as they did before that release. Clone Hero ghost and accent notes are marked from these volumes, so they are absent too, as they were then; tick the box and both come back.</li><li>**Kick notes are never varied, either way.** A kick's volume has to be read from the 20-120 Hz range, which is also where the bass guitar sits, so the reading followed the bassline as much as the drumming - a quiet kick under a loud bass note came out loud, and a firm kick under a rest came out quiet. Kick is written at one volume whatever the setting above says.</li><li>**Fewer notes that were never played.** When detection fires where nothing was struck, the note it invents has almost no energy behind it, because there was nothing there to measure. Snare and kick notes that come back that quiet are now dropped before the chart is written. Both were measured against a large library of community charts before being switched on. This runs whether or not note volume is enabled - the volume is still measured, it simply is not written into the chart. Hi-hat, crash, ride and tom were measured the same way and deliberately left alone.</li><li>**Cymbals played with a snare are no longer deleted.** A pass that removed cymbals landing at the same moment as a snare has been switched off. It was treating ordinary drumming as bleed - a snare hit together with a hi-hat and a crash is a normal thing to play. Checked against human-made charts, roughly nine of every ten cymbals it deleted were notes the chart wanted.</li><li>**Advice that pointed at something removed.** The Help tab suggested a Metal / Hard Rock / Post-Hardcore preset for heavy songs and the Detection Troubleshooter offered Metal as a fix for too many detected notes. Genre presets were removed in the previous release, so both pointed at nothing - and the Metal suggestion was backwards anyway, since that preset added notes rather than removing them.</li><li>**Twelve fixed bugs, each reproduced and verified before it was touched.** The MIDI Extractor dropped china, splash and 13-inch crash from every extracted file. The cleanup pass could swap two cymbals played at the same instant. The Preview tab timed every multi-track MIDI at 120 BPM, so notes drifted even though the BPM shown was right, and it refused chart files every other part of ParaKit opens. The Practice Mixer's Drums slider switched the backing drums back on during You-drum mode. Saved Audio to MIDI settings could be overwritten by an older snapshot, which then wrote the stale value back to disk. A batch run could destroy its own output when two difficulties of the same song overwrote each other's Clone Hero chart, both reporting success. Re-exporting a chart destroyed the existing file the moment the export began. Clone Hero covers were copied under a .png name whatever format they really were. Dragging one note before the start of a song shifted the whole saved chart later by the same amount. The retired genre presets still presented as live in several places. And the ML sensitivity slider does nothing across the bottom of its travel for some instruments - now stated in the conversion log rather than left to be discovered.</li></ul> |
